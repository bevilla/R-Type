...
bool			UnixUdpSocket::init()
{
  ...
  if ((pe = getprotobyname("UDP")) == NULL)
    return false;
  if ((_socket = socket(AF_INET, SOCK_DGRAM, pe->p_proto)) < 0)
    return false;
  _closed = false;
...
bool			UnixUdpSocket::bindSocket(uint16_t port)
{
  
}

bool		        UnixUdpSocket::sendData(const void *data, size_t size)
{
  if (send(_client ? _socket : _remoteSocket, (const char *)data, size, 0) < 0)
    return false;
  return true;
}

int			UnixUdpSocket::receive(void *&data, size_t &receive)
{
  char			tmp[RECVLEN];
  fd_set		rfds;
  struct timeval	tv;
